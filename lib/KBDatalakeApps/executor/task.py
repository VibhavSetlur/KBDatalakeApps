...
from annotation.annotation import run_rast, run_kofam, run_psortb
...
def task_kofam(filename_faa: Path, client):
    ...
    if filename_faa.exists() and filename_faa.is_file():
        ...
        output_annotation = _parent / f'{filename_faa.name[:-4]}_kofam.tsv'
        # run task
...
def task_psortb(filename_faa: Path, org_arg, client):
    ...
    if filename_faa.exists() and filename_faa.is_file():
        _parent = filename_faa.parent
        output_annotation = _parent / f'{filename_faa.name[:-4]}_psortb.tsv'
        # run task
